...
ROLE_NAME="sixspur-adminSmsRecipients-execution-role"
REGION="us-east-1"
PROFILE="sixspur"
...
if ! aws iam get-role --role-name "$ROLE_NAME" --profile "$PROFILE" >/dev/null 2>&1; then
  ...
  aws iam create-role \
    ...
    --profile "$PROFILE"
  echo "Attaching basic Lambda execution policy (CloudWatch logs)..."
...
aws iam put-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-name AdminSmsRecipientsPermissions \
