...
echo "Creating route: POST /adopt/apply"
aws apigatewayv2 create-route \
  --api-id "$API_ID" \
  --route-key "POST /adopt/apply" \
  --target "integrations/${INTEGRATION_ID}" \
  --profile "$PROFILE" --region "$REGION" >/dev/null || echo "Route may already exist, skipping."

echo "Creating route: POST /adopt/photos/presign"
aws apigatewayv2 create-route \
  --api-id "$API_ID" \
  --route-key "POST /adopt/photos/presign" \
  --target "integrations/${INTEGRATION_ID}" \
  --profile "$PROFILE" --region "$REGION" >/dev/null || echo "Route may already exist, skipping."
...
aws lambda add-permission \
  ...
  --statement-id apigateway-adopt-invoke \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/*/adopt/*" \
...
echo ""
echo "Done."
echo "Adoption application endpoint: ${API_ENDPOINT}/adopt/apply"
