...
aws lambda add-permission \
  ...
  --statement-id apigateway-news-invoke \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/*/news*" \
  --profile "$PROFILE" --region "$REGION" 2>/dev/null || echo "Permission already exists, skipping."

# Separate permission needed since /admin/news doesn't share the "/news*" prefix pattern
aws lambda add-permission \
  --function-name "$FUNCTION_NAME" \
  --statement-id apigateway-admin-news-invoke \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/*/admin/news*" \
  --profile "$PROFILE" --region "$REGION" 2>/dev/null || echo "Permission already exists, skipping."
...
echo ""
echo "Done."
echo "Public news endpoint: ${API_ENDPOINT}/news"
echo "Admin news endpoint: ${API_ENDPOINT}/admin/news"
