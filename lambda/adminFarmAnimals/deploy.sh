...
ROLE_NAME="sixspur-adminFarmAnimals-execution-role"
REGION="us-east-1"
PROFILE="sixspur"
...
aws iam put-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-name AdminFarmAnimalsPermissions \
  --policy-document file://execution-role-policy.json \
  --profile "$PROFILE"

ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/${ROLE_NAME}"
ENV_VARS="Variables={FARM_ANIMALS_TABLE=farm_animals,ASSETS_BUCKET=sixspurranch-assets,CDN_BASE=https://d1s8s7aw8vf5zu.cloudfront.net}"

if aws lambda get-function --function-name "$FUNCTION_NAME" --profile "$PROFILE" --region "$REGION" >/dev/null 2>&1; then
  echo "Function exists, updating code..."
  aws lambda update-function-code \
    --function-name "$FUNCTION_NAME" \
    --zip-file fileb://adminFarmAnimals.zip \
