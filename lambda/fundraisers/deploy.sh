...
ROLE_NAME="sixspur-fundraisers-execution-role"
REGION="us-east-1"
PROFILE="sixspur"
...
rm -f fundraisers.zip
zip -r fundraisers.zip index.js dynamo.js node_modules package.json
...
aws iam put-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-name FundraisersPermissions \
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --profile "$PROFILE" --region "$REGION" >/dev/null 2>&1; then
  ...
  aws lambda update-function-code \
    ...
    --zip-file fileb://fundraisers.zip \
