...
# Requires the JWT authorizer from 00-cognito-setup.txt step 4 to already
# exist (same authorizer used for /donor/* routes). Pass its ID:
...
aws lambda add-permission \
  ...
  --statement-id apigateway-donate-invoke \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/*/donate/*" \
  --profile "$PROFILE" --region "$REGION" 2>/dev/null || echo "Permission already exists, skipping."

echo ""
echo "Done. Both /donate/* routes now require a valid Cognito JWT (donor must be logged in to donate)."
