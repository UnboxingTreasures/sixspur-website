...
# exist. Pass its ID as the first argument:
...
aws lambda add-permission \
  ...
  --statement-id apigateway-donors-invoke \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/*/donor/*" \
  --profile "$PROFILE" --region "$REGION" 2>/dev/null || echo "Permission already exists, skipping."

echo ""
echo "Done. All /donor/* routes now require a valid Cognito JWT."
