...
FUNCTION_NAME="sixspur-newsletter"
...
echo "Creating route: POST /api/newsletter"
aws apigatewayv2 create-route \
  --api-id "$API_ID" \
  --route-key "POST /api/newsletter" \
...
aws lambda add-permission \
  ...
  --statement-id apigateway-newsletter-invoke \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/*/api/newsletter" \
...
echo ""
echo "Done."
echo "Newsletter endpoint: ${API_ENDPOINT}/api/newsletter"
