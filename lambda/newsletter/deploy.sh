...
ROLE_NAME="sixspur-newsletter-execution-role"
REGION="us-east-1"
PROFILE="sixspur"
...
rm -f newsletter.zip
zip -r newsletter.zip index.js node_modules package.json
...
if ! aws iam get-role --role-name "$ROLE_NAME" --profile "$PROFILE" >/dev/null 2>&1; then
  ...
  aws iam put-role-policy \
    --role-name "$ROLE_NAME" \
    --policy-name NewsletterPermissions \
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --profile "$PROFILE" --region "$REGION" >/dev/null 2>&1; then
  ...
  aws lambda update-function-configuration \
    --function-name "$FUNCTION_NAME" \
    --environment "Variables={SUBSCRIBERS_TABLE=subscribers}" \
    --profile "$PROFILE" --region "$REGION"
else
