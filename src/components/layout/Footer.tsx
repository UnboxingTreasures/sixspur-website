...
export default function Footer() {
  // UPDATED (Session 20) -- FIXED a real bug: this used to import
  // categories from a static shopItems.json seed file bundled at BUILD
  // time. The comment above it claimed "derived from the actual
  // product data, not hardcoded", which was true in the sense that it
  // wasn't literally typed out by hand -- but it was still derived
  // from a stale snapshot, not the live shop_items table. Confirmed by
  // Jay: after today's data cleanup wiped shop_items to empty, this
  // footer kept showing category links (hats, tumblers, etc.) from
  // products that no longer exist, because the JSON file never
  // changes unless the site is rebuilt with a new seed file. Now
  // fetches the real, live public /shop endpoint (same one the actual
  // shop page uses) instead, so this genuinely reflects current
  ...
  useEffect(() => {
    fetch(`${API_URL}/shop`)
      .then((res) => res.json())
      .then((data) => {
        const items: ShopItem[] = Array.isArray(data.items) ? data.items : [];
        const categories = Array.from(new Set(items.map((item) => item.category)));
        setShopCategories(categories);
      })
      .catch((err) => console.error('Footer: failed to load shop categories:', err));
  }, []);

  return (
    <footer className="bg-spur-tan-light text-spur-black">
      <div className="h-1 bg-spur-orange w-full" />
      <div className="max-w-6xl mx-auto px-6 py-14">

        {/* Top grid */}
        <div className="grid grid-cols-2 md:grid-cols-6 gap-10 pb-10 border-b border-spur-tan">

          {/* Brand */}
          <div className="col-span-2">
            <div className="flex items-center gap-2 mb-3">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 928 1130" className="w-7 h-8 flex-shrink-0">
                <g fill="#111111">
              ...
              <span className="font-semibold text-sm text-spur-black">Six Spur Ranch and Rescue</span>
            </div>
            <p className="text-spur-black text-xs leading-relaxed mb-2">
              An animal rescue and sanctuary in Maud, Texas.
            </p>
          </div>
          ...
          {/* Give */}
          <div>
            <h4 className={headingClass}>Give</h4>
            <div className="flex flex-col gap-2">
          ...
          {/* Shop -- categories now derived from the LIVE /shop
              endpoint (see useEffect above), not a static seed file.
              If there's currently no inventory (shopCategories is
              empty), fall back to a single generic link straight to
              /shop rather than rendering an empty column with just a
          ...
          {/* Ranch */}
          <div>
            <h4 className={headingClass}>Ranch</h4>
            <div className="flex flex-col gap-2">
              <Link href="/about" className={linkClass}>Our story</Link>
              <Link href="/farm-animals" className={linkClass}>The farm family</Link>
              <Link href="/news" className={linkClass}>News and updates</Link>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-8">
          <div>
            <p className="text-spur-black text-xs">© {new Date().getFullYear()} Six Spur Ranch and Rescue. All rights reserved.</p>
            <p className="text-spur-black text-xs mt-1">Six Spur Ranch and Rescue is a registered 501(c)(3) | EIN: 41-4123317. Your contribution is tax-deductible to the extent allowed by law.</p>
            <div className="flex gap-4 mt-2">
              <Link href="/privacy" className="text-spur-black text-xs hover:text-spur-orange transition-colors">Privacy Policy</Link>
              <Link href="/data-deletion" className="text-spur-black text-xs hover:text-spur-orange transition-colors">Data Deletion</Link>
              <Link href="/terms" className="text-spur-black text-xs hover:text-spur-orange transition-colors">Terms of Service</Link>
            </div>
          </div>

          {/* Social icons */}
          <div className="flex gap-4 items-center">
            <a href="https://www.facebook.com/profile.php?id=61591276504389" target="_blank" rel="noreferrer" aria-label="Facebook"
              className="text-spur-black hover:text-spur-orange transition-colors flex">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
              </svg>
            </a>
            <a href="https://www.instagram.com/sixspurrescue/" target="_blank" rel="noreferrer" aria-label="Instagram"
              className="text-spur-black hover:text-spur-orange transition-colors flex">
            ...
            <a href="https://www.tiktok.com/@sixspur" target="_blank" rel="noreferrer" aria-label="TikTok"
              className="text-spur-black hover:text-spur-orange transition-colors flex">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.75a4.85 4.85 0 0 1-1.01-.06z"/>
              </svg>
            </a>
            <a href="https://www.youtube.com/@SixSpurRanchandRescue" target="_blank" rel="noreferrer" aria-label="YouTube"
              className="text-spur-black hover:text-spur-orange transition-colors flex">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
